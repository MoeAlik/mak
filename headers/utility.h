...
struct pair {
    ...
    constexpr pair(U1&& x, U2&& y)
        : first { mak::forward<U1>(x) }
        , second { mak::forward<U2>(y) } {}
    ...
    constexpr pair(const pair<U1, U2>&& p)
        : first { mak::forward<U1>(p.first) }
        , second { mak::forward<U2>(p.second) } {}
...
const typename tuple_element<I, pair<T1, T2>>::type& get(const pair<T1, T2>&) {
    throw "wijhak";
}

template <typename T1, typename T2>
const typename tuple_element<0, pair<T1, T2>>::type& get(const pair<T1, T2>& p) {
    return p.first;
}

template <size_t I, typename T1, typename T2>
const typename tuple_element<I, pair<T1, T2>>::type&& get(const pair<T1, T2>&& p) {
    if constexpr (std::is_same_v<typename tuple_element<I, pair<T1, T2>>::type, T1>) {
        return p.first;
    } else {
        return p.second;
    }
